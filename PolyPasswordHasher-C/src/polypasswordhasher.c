...
int main(void)
{
  ...
  // we need 3 accounts for bootstrap but we have four total
  uint8 threshold = 3;    
  ...
  printf("Alice protector account created\n");
  pph_create_account(context, "Bob", strlen("Bob"),
                       "i.secretly.love.eve",strlen("i.secretly.love.eve"),1);
  printf("Bob protector account created\n");
  pph_create_account(context, "Carol", strlen("Carol"),
                       "abcdefg",strlen("abcdefg"),1);
  printf("Carol protector account created\n");
  pph_create_account(context, "Dan", strlen("Dan"),
                       "badpassword",strlen("badpassword"),1);
  printf("Dan protector account created\n");
  ...
  printf("Eve shielded account created\n");
  ...
  printf("current context destroyed\n");
  ...
  printf("context reloaded from disk, now we are in bootstrap mode\n");
  ...
  if(pph_create_account(context, "trudy", strlen("trudy"), "I'm.trudy", 
    ...
    printf("!!! This shouldn't happen, can't make protector accounts during bootstrap \n");
  }

  // try shielded account
  if(pph_create_account(context, "jane", strlen("jane"), "helloworld", 
                            strlen("helloworld"), 0) == PPH_CONTEXT_IS_LOCKED){
    printf("Sorry, we cannot create accounts at this time\n");
  }else{
    printf("We made a shielded account which should be allowed\n");
  }
  

  printf("logging in protector entries...\n");
  ...
  username_lengths[2] = strlen("Carol");
  //username_lengths[3] = strlen("Dan");
  ...
  password_lengths[2] = strlen("abcdefg");
  //password_lengths[3] = strlen("wrong_password");
  ...
  pph_unlock_password_data(context, 3, usernames, username_lengths, passwords, password_lengths);

  // now the data is available. We can create accounts now.
  pph_create_account(context, "carl", strlen("carl"), "verysafe", 
                                                        strlen("verysafe"),0);
  printf("carl account created\n");
  ...
  if(pph_check_login(context, "carl", strlen("carl"), "verysafe",
    ...
    printf("welcome back carl, shielded login works\n"); 
  }else{
    printf("you are not carl\n");
  }

  if(pph_check_login(context, "Dan", strlen("Dan"), "badpassword",
                                          strlen("badpassword")) == PPH_ERROR_OK){
    printf("welcome back Dan, protector checking works\n"); 
  }else{
    printf("Dan cannot be logged in, shares may be corrupted\n");
  }
  
  // we should now store the context and free the data before leaving
  pph_store_context(context,"securepasswords");
  printf("storing context again\n");
  pph_destroy_context(context);
