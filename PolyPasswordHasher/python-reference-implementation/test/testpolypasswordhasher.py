...
class TestPolyPasswordHasher(TestCase):
  ...
  def test_polypasswordhasher(self):
    ...
    # password file...
    with polypasswordhasher.PolyPasswordHasher(threshold = THRESHOLD, passwordfile = None) as pph:
      ...
      pph.create_account('rahul','ramesh',1)
    ...
    # __exit__ method is called after "with" statement
    # pph = None

    # # let's load it back in
    with polypasswordhasher.PolyPasswordHasher(threshold = THRESHOLD,passwordfile = 'securepasswords') as pph:
      ...
      try: 
        print (pph.is_valid_login('alice','kitten'))
      except ValueError:
        pass
      else:
